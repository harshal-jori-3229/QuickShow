...
function MovieDetails() { 
  ...
  return show ? (
      <div className='px-6 md:px-16 lg:px-40 pt-8 md:pt-12 mt-40'>
        <div className='flex flex-col md:flex-row gap-8 max-w-6xl mx-auto'>
          <img 
            src={show.movie.poster_path} 
            alt="" 
            className='md:mx-auto rounded-xl h-[26rem] max-w-[17.5rem] object-cover'
          />
        ...
        <div className="overflow-x-auto no-scrollbar mt-8 pb-4">
          <div className="flex items-center gap-6 w-max px-4 cursor-pointer">
            {show.movie.casts.slice(0, 12).map((cast, index) => (
              <div key={index} className="flex flex-col items-center text-center">
                ...
                />
                <p className="mt-2 text-sm font-medium text-gray-200">{cast.name}</p>
              </div>
            ))}
          </div>
        </div>
